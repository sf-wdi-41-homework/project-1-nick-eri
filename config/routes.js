...
// Home page route
router.route('/').get(controllers.statics.home);
...
router.route('/newprofile').get(authenticatedUser, controllers.auth.newProfile);

// Route once new profile updated
router
  .route('/newprofile/:id/complete')
...
// Handles logout get
router.route('/logout').get(controllers.auth.getLogout);

// Handles User List page
router.route('/users').get(controllers.users.show);

// Handles user index page
router.route('/users/:username').get(controllers.users.index);
...
router.route('/workspaceitems/create').post(controllers.workspaceItems.create);

// Handles deletion of certain list items
router.route('/workspaceItems/:id').delete(controllers.workspaceItems.destroy);

// Handles search function for AJAX calls
router.route('/software/search/:software').get(controllers.softwares.search);
